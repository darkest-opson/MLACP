...
for p in path:
    ...
    commands = {
        ...
        'DPC': f'python {ifeature_path}iFeature.py --type DPC --file {input_fasta} --out {input_fasta[:-6]+"_3_"+output_files["DPC"]}',
        'CTDT': f'python {ifeature_path}iFeature.py --type CTDT --file {input_fasta} --out {input_fasta[:-6]+"_3_"+output_files["CTDT"]}',
        'AAC': f'python {ifeature_path}iFeature.py --type AAC --file {input_fasta} --out {input_fasta[:-6]+"_3_"+output_files["AAC"]}',
    }
    ...
    dfs = {feature: pd.read_csv(input_fasta[:-6]+"_3_"+output_files[feature], sep='\t') for feature in output_files}
    ...
    combined_df.to_csv(f'.\\data\\Combined_Features_{p[:-6]}3.csv', index=False)

    print("Feature extraction and combination completed and saved to CSV file.")

df_neg = pd.read_csv(".\\data\\Combined_Features_negative3.csv")
df_pos = pd.read_csv(".\\data\\Combined_Features_positive3.csv")

# Concatenate the DataFrames vertically
merged_df = pd.concat([df_neg, df_pos], axis=0, ignore_index=True)

# Save the merged DataFrame to a new CSV file
merged_df.to_csv('.\\data\\datasets\\combination3.csv', index=False)
print("code done------>")
